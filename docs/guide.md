...
myst:
  substitutions:
    CDP: "*Cookiecutter Django Package*"
---

# User Guide

This is the user guide for [Cookiecutter Django Package], a Django
package template adapted from the [Hypermodern Python Cookiecutter].
...
Install [Poetry] by following the instructions at [install.python-poetry]
as shown here:
...
:::{note}
Do **not** initialize with a `README.md`, `LICENSE`, or `.gitignore`.
...
Then push contents of your local git repository to the remote repository on GitHub:

```console
$ git push --set-upstream origin main
```

If you get permission errors and are trying to push to an organization:

```console
$ git push https://your_username@github.com/<org_username>/<project>.git
```
...
- - `src/<package_name>`
  - Django package
- - `compose`
  - Contains Docker Compose files (optional)
- - `example_project`
  - Example Project which incorporates your Django package
- - `tests`
  - Test suite
- - `docs`
  - Documentation
...
- - `src/<package_name>/migrations/__init__.py`
  - Migrations module initialization
- - `src/<package_name>/*`
  - Django Package source files
...
- - `terminology.md`
  - Stub for terminology used in the package
...
- - `docker-compose.yml`
  - Configuration for [Docker Compose](how-to-run-your-code-in-docker-compose)
- - `docs/conf.py`
...
You can find the initial Django package in your generated project
...
: A stub for adding terminology in any package with a specialized focus.
...
- _Development dependencies_ are only required by developers working on your code.
  ...
  or to build documentation. These dependencies are not a part of distribution packages,
...
:::{note}
Dependencies in the {{ CDP }} are managed by [Dependabot](dependabot-integration).
...
You can create environments explicitly with the [poetry env] command,
specifying the desired Python version. This allows you to create an environment
for every Python version supported by your project, and easily switch between them:
...
This section gives a short overview of how you can build and distribute your package
from the command line, using the following Poetry commands:
...
which uploads the distribution packages to your account on [PyPI],
...
The {{ CDP }} uses the [poetry export] command
...
:::{note}
Pre-commit only runs agains files that have been staged in a git repo. If you see
"(no files to check)Skipped", it may be because you have not staged any files.
Stage files with `git add <files>` and try again.
:::
...
Python-language hooks in the {{ CDP }} are not managed by pre-commit.
...
:::{note}
Python-language hooks in the {{ CDP }} are defined as [system hooks][pre-commit system hooks].
...
Furthermore, the {{ CDP }} defines Python-language hooks as [repository-local hooks][pre-commit repository-local hooks].
...
The {{ CDP }} comes with a pre-commit configuration consisting of the following hooks:
...
The {{ CDP }} activates the [Black profile][isort black profile] for compatibility with the Black code formatter.
...
The {{ CDP }} integrates Flake8 via a [pre-commit] hook,
...
Each section also notes any configuration settings applied by the {{ CDP }}.

### Overview of available plugins

Flake8 comes with a rich ecosystem of plugins.
The following table lists the Flake8 plugins used by the {{ CDP }},
...
The {{ CDP }} disables the following errors and warnings
...
The {{ CDP }} selects the recommendations of the
...
The {{ CDP }} also enables Bugbear's `B9` warnings,
...
of your Django package against a configured limit.
The tool is included with [Flake8].

[Error codes][mccabe codes] are prefixed by `C` for "complexity".

The {{ CDP }} limits code complexity to a value of 10.
...
The {{ CDP }} allows one-line docstrings without function signatures.
...
The {{ CDP }} disables `S101` (use of assert) for the test suite,
...
The {{ CDP }} defines the following workflows:
...
First, [install Docker Engine], which includes Docker Compose.

Then, build and run the example project, which installs a local copy of your Django package
...
[install.python-poetry]: https://github.com/python-poetry/install.python-poetry.org
